...
func AsBW(err error) *BWStatus {
	...
	if !ok {
		log.Criticalf("You have discovered an uncoverted error mode.")
		log.Criticalf("In the alpha series, we exit the router so you notice it.")
		log.Criticalf("Please send an email to m.andersen@cs.berkeley.edu with")
		log.Criticalf("the following information:")
		log.Criticalf(" UNEXPORTED ERROR: %+v", err)
		panic(err)
	}
	return bwerr
}
