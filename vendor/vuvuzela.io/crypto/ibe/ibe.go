...
func GetEncryptSecret(random io.Reader, pub *MasterPublicKey, id []byte) (rpb []byte, secret []byte) {
	q := new(bn256.G2).HashToPoint(id)
	...
	return rp.Marshal(), sk[:]
}
func GetDecryptSecret(priv *IdentityPrivateKey, rpb []byte) []byte {
	rp := new(bn256.G1)
	_, ok := rp.Unmarshal(rpb)
	if !ok {
		return nil
	}
	e := bn256.Pair(rp, priv.d)
	shake := sha3.NewShake256()
